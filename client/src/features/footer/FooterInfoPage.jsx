  ...
  return (
    <header className="sticky top-0 z-40 border-b border-[color:var(--border-soft)] bg-[color:rgba(9,17,29,0.94)] px-5 py-4 text-[color:var(--text-primary)] shadow-[0_18px_44px_rgba(0,0,0,0.28)] backdrop-blur-xl sm:px-8 lg:px-14">
      <div className="mx-auto flex max-w-[1320px] flex-wrap items-center justify-between gap-4">
        ...
        <nav className="flex flex-wrap items-center gap-4 text-sm font-extrabold text-[color:var(--text-secondary)] sm:gap-6 sm:text-base">
          <Link to="/plans/individual" className="transition hover:text-[color:var(--text-primary)]">
            Gói Premium
          </Link>
          <Link to={appPaths.support} className="transition hover:text-[color:var(--text-primary)]">
            Hỗ trợ
          </Link>
          <Link to="/mobile-app" className="inline-flex items-center gap-2 transition hover:text-[color:var(--tertiary)]">
            <DownloadOutlined />
            Tải xuống
          </Link>
          <span className="hidden h-7 w-px bg-white/10 md:block" />
          <Link to={appPaths.auth.register} className="transition hover:text-[color:var(--text-primary)]">
            Đăng ký
          </Link>
...
function FooterInfoPage({ pageKey }) {
  const page = footerPageData[pageKey]
      ...
      <main>
        <section className="relative overflow-hidden px-5 py-16 sm:px-8 sm:py-20 lg:px-14">
          ...
          <div className="relative mx-auto max-w-[1040px]">
            ...
            <h1 className="font-display text-[3rem] font-extrabold leading-[1.04] tracking-[-0.06em] sm:text-[4.4rem]">
              {page.title}
            </h1>
