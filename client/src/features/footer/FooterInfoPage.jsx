  ...
  return (
    <header className="sticky top-0 z-40 border-b border-[color:var(--border-soft)] bg-[color:rgba(9,17,29,0.94)] px-5 py-4 text-[color:var(--text-primary)] shadow-[0_18px_44px_rgba(0,0,0,0.28)] backdrop-blur-xl sm:px-8 lg:px-14">
      <div className="mx-auto flex max-w-[1320px] flex-wrap items-center justify-between gap-4">
        ...
        <nav className="flex flex-wrap items-center gap-4 text-sm font-semibold text-[color:var(--text-secondary)] sm:gap-6 sm:text-base">
          <Link to="/plans/individual" className="top-nav-link">
            Gói Premium
          </Link>
          <Link to={appPaths.support} className="top-nav-link">
            Hỗ trợ
          </Link>
          <Link to="/mobile-app" className="top-nav-link inline-flex items-center gap-2">
            <DownloadOutlined />
            Tải xuống
          </Link>
          <span className="hidden h-7 w-px bg-white/10 md:block" />
          <Link to={appPaths.auth.register} className="top-nav-link">
            Đăng ký
          </Link>
